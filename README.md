#4.06a
修改了run_target，
除此之外，需要思考是否修改其它结构，包括和socket的通讯等
...
neuzz舍弃了afl的queue机制，直接面向io进行读取和变异

但是network功能后，原有的io机制不能满足信息的存储，因此本次commit添加了 read_testcases，即建立队列
